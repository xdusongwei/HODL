...
class EnablePosition(TelegramBotBase):
    ...
    async def enable_start(self, update, context):
    ...
    async def enable_select(self, update, context):
    ...
    async def enable_decision(self, update, context):
    ...
    async def enable_confirm(self, update, context):
        text = update.message.text
        user_id = update.message.from_user.id
        session = self._get_session(user_id=user_id)
        match text:
    ...
    def handler(cls):
        ...
        handler = ConversationHandler(
            ...
            states={
                o.K_EN_SELECT: [MessageHandler(Regex(r'^(\d+)$'), o.enable_select)],
                o.K_EN_DECISION: [
