...
class RiskControl:
    ...
    如果判断为异常，异常信息会记录到持仓状态，Store对象将自杀以拒绝主体(下单)工作，除非人工核对完成，并删除了持仓状态中的风控字段，再启动进程
    模块围绕持仓状态去做一些判断工作，出现的问题必须人工介入，为防呆，风控异常的Store对象即便重启进程仍不能正常工作
    注意，像下单动作发到broker中间出了问题，其不围绕持仓状态数据，不是风控模块管理范围，而且这种情况没有任何防呆设计，
    没有核对系统异常状态信息即重启可能会重放订单！

    LSOD
    lsod(上一次提交订单日)提供了一组机制，保证确认前一个交易日内的订单信息周期有效结束
    ...
    基于此，来保障下单日系统在收盘阶段时仍然活着，且更新过当日订单，订单里面的数值不会再变化，订单的生命周期可以确认结束
    ...
    def _order_day_times_check(self, order: Order):
        ...
        if times == 0:
            ...
            if not order.limit_price:
                ...
                if order.direction == 'BUY' and diff > 0 and cash_amount / order.limit_price < diff:
                    raise RiskControlError(f'当日首单检查时,现金无法完全买入总买卖量差量的股票: 差额{diff:,}股, 现金{cash_amount:,}')
                if order.direction == 'BUY' and order.qty * order.limit_price > cash_amount:
