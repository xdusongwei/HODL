...
class BasePriceDayLow(TelegramBotBase):
    ...
    def bpdl_start(self, update, context):
    ...
    def bpdl_select(self, update, context):
    ...
    def bpdl_decision(self, update, context):
    ...
    def bpdl_confirm(self, update, context):
        text = update.message.text
        user_id = update.message.from_user.id
        session = self._get_session(user_id=user_id)
        match text:
    ...
    def handler(cls):
        ...
        handler = ConversationHandler(
            ...
            states={
                o.K_BPDL_SELECT: [MessageHandler(Filters.regex(r'^(\d+)$'), o.bpdl_select)],
                o.K_BPDL_DECISION: [
