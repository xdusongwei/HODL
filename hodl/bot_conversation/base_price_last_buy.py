...
class BasePriceLastBuy(TelegramBotBase):
    ...
    def bpe_start(self, update, context):
        ...
        update.message.reply_text(
            f'你正在尝试设置修改[基准价格买回](basePriceLastBuy)的设定，指定持仓未有任何买卖单时会有效执行。 '
            f'[开启]时，[基准价格]可以是[临时基准价], [昨收价]和[上次买入平仓价]中最小的; [关闭]时，[基准价格]只能是[昨收价]。'
            f'选择需要操作的标的序号\n'
    ...
    def bpe_decision(self, update, context):
        ...
        update.message.reply_text(
            f'确认针对{session.position.display}的[基准价格买回]改动为{new_icon}? 使用命令 /confirm 来确认',
            reply_markup=ReplyKeyboardRemove(),
        )
        return self.K_BPE_CONFIRM
    ...
    def bpe_confirm(self, update, context):
        ...
        match text:
            case '/confirm':
                update.message.reply_text(
                    f'已经确认改动{session.position.display}的[基准价格买回]',
                    reply_markup=ReplyKeyboardRemove(),
                )
                try:
    ...
    def handler(cls):
        ...
        handler = ConversationHandler(
            ...
            states={
                o.K_BPE_SELECT: [MessageHandler(Filters.regex(r'^(\d+)$'), o.bpe_select)],
                o.K_BPE_DECISION: [
