...
class LsodTestCase(HodlTestCase):
    def test_lsod(self):
        ...
        store = SimulationBuilder.from_symbol(symbol='TEST', ticks=ticks, auto_run=False)
        with store:
            store.run()
            state = store.state
        ...
        store = SimulationBuilder.resume(store=store, ticks=ticks, auto_run=False)
        with store:
            store.run()
            state = store.state
            assert state.is_lsod_today
    ...
    def test_lsod_crypto(self):
        ...
        store = SimulationBuilder.from_symbol(symbol='BTC', ticks=ticks, auto_run=False)
        with store:
            store.run()
            state = store.state
            assert state.is_lsod_today
